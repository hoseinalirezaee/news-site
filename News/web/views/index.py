...
class Index(generic.TemplateView):
    ...
    def get_last_news(self):
        last_news_queryset = models.Post.objects.all().filter(main_image__isnull=False) \
                                 .order_by('-date_posted')[:self.last_news_size]
        return last_news_queryset

    def get_last_news_with_category(self):
        root_categories = models.Category.objects.all().filter(parent__isnull=True).exclude(
            title='سایر'
        )
        last_news = {}
